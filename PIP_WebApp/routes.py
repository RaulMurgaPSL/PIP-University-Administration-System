...
def edit_university(university_id):
    ...
    if university:
        ...
        return render_template(
            "edit/edit_university.html", form=form, university_id=university_id
        )
...
def addcollege():
    ...
    if form.validate_on_submit():
        university = db.session.query(models.University).filter(
            models.University.name == form.university.data
        )[0]
...
def edit_college(college_id):
    ...
    if college:
        ...
        return render_template(
            "edit/edit_college.html", form=form, college_id=college_id
        )
...
def addstream():
    ...
    if form.validate_on_submit():
        college_id = (
            db.session.query(models.College)
            .filter(models.College.name == form.college.data)[0]
            .id
        )
        college = college = db.session.query(models.College).filter(
            models.College.name == form.college.data
        )[0]
        university_id = (
            db.session.query(models.College)
            .filter(models.College.id == college_id)[0]
            .university_id
        )
        university = db.session.query(models.University).filter(
...
def delete_stream(stream_id):
    ...
    if stream:
        if form.validate_on_submit():
            ...
            return redirect(url_for("streams"))
        return render_template(
            "delete/delete_stream.html", form=form, stream_id=stream_id
        )
    flash(f"Stream with id {stream_id} does not exit")
    return redirect(url_for("streams"))
...
def addcourse():
    ...
    if form.validate_on_submit():
        ...
        stream = db.session.query(models.Stream).filter(
            models.Stream.name == form.stream.data
            and models.Stream.College_id == college_id
        )[0]
        stream_id = stream.id
...
def delete_course(course_id):
    ...
    if course:
        if form.validate_on_submit():
            ...
            return redirect(url_for("courses"))
        return render_template(
            "delete/delete_course.html", form=form, course_id=course_id
        )
    flash(f"Course with id {course_id} does not exit")
    return redirect(url_for("courses"))
...
def marksheet(student_id):
    ...
    if courses.first() == 0:
        ...
        gpa = db.session.query(
            db.func.avg(models.Course.grade).label("average")
        ).filter(models.Course.student_id == student_id)[0][0]
        gpa = float(f"{gpa:.2f}")
        ...
        if marksheet.__eq__([]):
            marksheet = models.Marksheet(
                student_id=student_id, gpa=gpa, courses=courses.all()
            )
...
def edit_student(student_id):
    ...
    if student:
        ...
        return render_template(
            "edit/edit_student.html", form=form, student_id=student_id
        )
...
def search():
    ...
    try:
        if form.validate_on_submit():
            print(
                form.search_target.data, form.search_filter.data, form.search_input.data
            )
            ...
            joined = db.session.query(target, filter).filter(
                filter.id == getattr(target, "_".join([filter_, "id"]).lower())
            )
        ...
        flash(
            "Try to search from particular [smaller] and reference from universal [bigger]."
        )
        flash(
            "For instance, search for students (particular) filtering by College (universal)."
        )
